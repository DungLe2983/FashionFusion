...
const CustomizeProducts = (pros) => {
  ...
  const fetchItems = async () => {
    ...
    try {
      for (const i of pros.data) {
        const response = await fetch(`/api/products/${pros.id}/${i._id}/`, {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        });
  ...
  const handleSizeClick = (sizeDescription, sizeId) => {
    ...
    const filteredItems = items.filter((item) => item.size_id._id === sizeId);
    ...
    if (!displayedSizes.includes(size.size_id._id)) {
      ...
      return (
        <button
          ...
          onClick={() =>
            handleSizeClick(size.size_id.description, size.size_id._id)
          }
        >
          {size.size_id.name}
        </button>
          ...
          <>
            <p className="font-semibold text-sm mt-6">
              ...
              <span className="text-gray-500 ">
                {selectedSize &&
                  items.find((s) => s.size_id.description === selectedSize)
                    .size_id.description}
              </span>
            </p>
            ...
            <p className="font-semibold text-sm mt-6">
              ...
              <span className="text-gray-500 ml-2">
                {selectedColor &&
                  items.find((s) => s.color_id.description === selectedColor)
          ...
          <>
            ...
            <div className="flex flex-row items-center my-2">
              ...
              <p className="mx-5 text-sm">
                Còn lại{" "}
                <span className=" text-orange-500 font-bold">{stock}</span> sản
                phẩm
              </p>
            </div>
